...
const Admin = () => {
  ...
  const [activityStats, setActivityStats] = useState(getActivityStats());
  ...
  const menuItems = [
    ...
    { id: "activity", label: "Track Record", icon: BarChart3 },
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          <div className="max-w-7xl mx-auto space-y-8">
            {activeTab === "overview" && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                  <StatsCard
                    title="Total Logins"
                    value={activityStats.totalLogins.toLocaleString()}
                    change={activityStats.last7Days.login ? `+${activityStats.last7Days.login} (7d)` : undefined}
                    icon={Activity}
                    color="text-blue-400"
                  />
                  <StatsCard
                    title="Sign Ups"
                    value={activityStats.totalSignups.toLocaleString()}
                    icon={Users}
                    color="text-purple-400"
                  />
                  <StatsCard
                    title="DSA Activity"
                    value={activityStats.totalDsaActivity.toLocaleString()}
                    change={activityStats.last7Days.dsa ? `+${activityStats.last7Days.dsa} (7d)` : undefined}
                    icon={Code2}
                    color="text-emerald-400"
                  />
                  <StatsCard
                    title="Typing Activity"
                    value={activityStats.totalTypingActivity.toLocaleString()}
                    change={activityStats.last7Days.typing ? `+${activityStats.last7Days.typing} (7d)` : undefined}
                    icon={Type}
                    color="text-orange-400"
                  />
                  <StatsCard
                    title="Unique Users"
                    value={activityStats.uniqueUsers.toLocaleString()}
                    icon={TrendingUp}
                    color="text-cyan-400"
                  />
                </div>
                          ...
                          .map((entry) => (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                                  entry.type === "signup"
                                    ...
                                    : entry.type === "login" || entry.type === "admin_login"
                                    ? "bg-blue-500/20 text-blue-400"
                                    : entry.type.startsWith("dsa")
                                    ? "bg-emerald-500/20 text-emerald-400"
                                    : entry.type.startsWith("typing")
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate">{entry.name || entry.email || "Anonymous"}</p>
            ...
            {activeTab === "activity" && (
              <div className="bg-[#0F1422] border border-white/5 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold">Full Track Record</h3>
                  <button
                ...
                <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 bg-[#0F1422] z-10">
                      ...
                      {activityLog.length === 0 ? (
                        <tr>
                          <td colSpan={4} className="p-8 text-center text-gray-500">
                            No activity recorded yet. Users will appear here when they log in, do DSA, or typing.
                          </td>
                        </tr>
                      ) : (
                        [...activityLog]
                          ...
                          .map((entry) => (
                            <tr key={entry.id} className="hover:bg-white/5 transition-colors">
                              ...
                              <td className="p-4 font-medium">{entry.name || entry.email || "Anonymous"}</td>
                              <td className="p-4 text-gray-400 text-sm max-w-xs truncate">{entry.details || "-"}</td>
            ...
            {(activeTab === "contests" || activeTab === "settings") && (
              <div className="bg-[#0F1422] border border-white/5 rounded-2xl p-6">
                <div className="text-center py-20 text-gray-500">
